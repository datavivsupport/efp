...
const CsDocumentsPage = ({ jobData: initialJob, user }) => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      ...
      vsl_initial_eta: ad.vsl_initial_eta ? dayjs(ad.vsl_initial_eta) : null,
      vsl_latest_eta: ad.vsl_latest_eta ? dayjs(ad.vsl_latest_eta) : null,
      vsl_etd: ad.vsl_etd ? dayjs(ad.vsl_etd) : null,
      pod_eta: ad.pod_eta ? dayjs(ad.pod_eta) : null,
      si_cut_off_date: ad.si_cut_off_date ? dayjs(ad.si_cut_off_date) : null,
      si_cut_off_time: ad.si_cut_off_time ? dayjs(ad.si_cut_off_time, "HH:mm") : null,
    });
  }, [initialJob, form, ad]);
              ...
              <Form.List name="placementRows">
                {(fields) => fields.map(({ key, name, ...restField }) => (
                  <Row key={key} gutter={16} align="middle">
                    ...
                    <Col xs={24} md={4}><Form.Item {...restField} name={[name, "special_remarks"]} label="Remarks"><Input disabled variant="filled" /></Form.Item></Col>
                  </Row>
                ))}
              </Form.List>
            </div>
          ...
          <Card className={Styles.card} bordered title={<CardHeader icon="fluent:box-24-filled" title="BOOKING DETAILS (TECHNICAL DOCUMENTS)" open={open.booking} onToggle={() => toggle("booking")} />}>
            <div style={{ display: open.booking ? "block" : "none" }}>
              <Row gutter={[16, 8]}>
                ...
                <Col xs={24} md={6}><Form.Item className={Styles.formLabel} label="Load List/SI Cut Off Date" name="si_cut_off_date"><DatePicker style={{ width: "100%" }} disabled format="DD-MM-YYYY" /></Form.Item></Col>
                <Col xs={24} md={6}><Form.Item className={Styles.formLabel} label="Load List/SI Cut Off Time" name="si_cut_off_time"><Input disabled variant="filled" /></Form.Item></Col>

...
export const buildApprovalDetails = (values) => ({
  ...
  si_cut_off_date: values.si_cut_off_date ? values.si_cut_off_date.tz("Asia/Dubai").format("YYYY-MM-DD") : null,
  si_cut_off_time: values.si_cut_off_date ? values.si_cut_off_date.tz("Asia/Dubai").format("HH:mm") : null,
...
export const buildCommonPayload = (values, fileState, extraState) => {
  ...
  const allDocs = [
    ...
    ...hcsFiles.map((f) => ({ ...f, doc_type: "HCS", category: "financial" })),
